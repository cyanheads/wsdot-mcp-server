 ...
 * package the server only carries transitively.
 */
interface PageableTool<TInput, TOutput> {
  format: (result: TOutput) => { text?: string; type: string }[];
  handler: (input: TInput, ctx: Context) => Promise<TOutput>;
  input: { parse: (value: unknown) => TInput };
  name: string;
}

export interface PaginationContractOptions<TInput, TOutput> {
  ...
  tool: PageableTool<TInput, TOutput>;
 ...
 */
export function describePaginationContract<TInput, TOutput>(
  options: PaginationContractOptions<TInput, TOutput>,
): void {
  const {
    tool,
    createContext,
